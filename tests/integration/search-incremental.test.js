...
async function main() {
  ...
  try {
    ...
    assert.equal(built.features.annIndex, true, "index should build an ANN candidate index");
    ...
    assert.equal(built.vectorStore.ann.exists, true, "vector store should report ANN index status");
    ...
    assert.equal(incr.vectorStore.exists, true, "incremental index should refresh the vector store");
