...
- Searchable receipt metadata for model, tool, file, event type, and timestamp.
- Agent write diff previews.
- In-chat diff cards with explicit apply buttons.
- Validated recipe JSON loading with duplicate ID rejection.
- 25+ reusable JSON recipes, including prompt-injection review and meeting follow-up email.
...
- Cloud account sync by default
- Hidden file access outside the workspace
- Mandatory package-manager install
