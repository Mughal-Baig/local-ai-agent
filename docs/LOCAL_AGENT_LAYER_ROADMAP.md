...
| Codex Epic AR first-run pass | T295-T300 | Added the guided first-run product path: workspace/model choices, deterministic local typo-fix sample, action-led empty states, model-missing-only setup prompts, use-your-own-project handoff, privacy-visible local first-run telemetry, UI/tests/docs/release coverage, and repo eval hooks. |
...
- T301: continue with daily agent workflow now that Epic AR first-run retention is complete.
...
Next code target: daily agent workflow. Prioritize T301 persistent conversation UI, then T302 one-screen run review and T303 continue-last-run.
...
- [x] T295 Build a guided first-run wizard: choose workspace, choose model, run first task.
- [x] T296 Add a sample "fix a typo safely" task that always succeeds locally.
- [x] T297 Add empty states that show the next useful action instead of documentation blocks.
- [x] T298 Add model download/setup prompts only when the model is actually missing.
- [x] T299 Add "use your own project" handoff after the sample task completes.
- [x] T300 Add first-run telemetry that is local-only and visible in the privacy dashboard.
