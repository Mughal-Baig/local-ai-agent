...
async function main() {
  ...
  checks.push(await check("MCP stdio server exists", async () => includes("mcp/server.js", ["tools/list", "explicit MCP approval"]) && includes("tests/integration/mcp-server.test.js", ["tools/list", "write_file", "MCP server integration test passed"])));
  ...
  checks.push(await check("Epic AQ one-command install path exists", async () => includes("src/setup-doctor.js", ["agenttrail.setup-doctor.v1", "checkOllama", "checkPort", "Default model available"]) && includes("src/cli.js", ["agenttrail doctor", "prepareFirstRunWorkspace", "agenttrail-workspace"]) && includes("docs/INSTALL.md", ["npx agenttrail", "brew install agenttrail", "ghcr.io/mughal-baig/agenttrail:latest", "Gatekeeper"]) && includes("docker-compose.yml", ["/data/workspace", "WORKSPACE_ROOT"]) && includes(".github/workflows/ci.yml", ["tests/integration/install-smoke.test.js"]) && includes("tests/integration/install-smoke.test.js", ["npm", "pack", "agenttrail", "doctor"])));
  checks.push(await check("Epic AR guided first-run path exists", async () => includes("src/config-admin.js", ["FIRST_RUN_SAMPLE_PATH", "guidedSteps", "first-run-telemetry"]) && includes("server.js", ["runFirstRunSampleTask", "use-own-project", "first-run-safe-typo.md"]) && includes("public/index.html", ["firstRunWorkspace", "runFirstRunSample", "useOwnProject"]) && includes("public/app.js", ["nextOnboardingAction", "useOwnProjectHandoff", "modelNameInstalled"]) && includes("src/privacy-controls.js", ["First-run telemetry", "firstRunTelemetry"]) && includes("tests/integration/config-admin.test.js", ["run-sample-task", "first-run-safe-typo.md"]) && includes("docs/GETTING_STARTED.md", ["Run safe sample", "first-run/sample-typo.md"])));
  ...
  checks.push(await check("Epic AM per-chat token time accounting exists", async () => includes("src/accounting-routing.js", ["agenttrail.usage-record.v1", "buildUsageRecord", "tokensPerSecond"]) && includes("server.js", ["usage-accounting", "sendEvent(res, \"accounting\"", "inputTokens"]) && includes("public/app.js", ["eventName === \"accounting\"", "Usage "])));
  checks.push(await check("Epic AM usage dashboard exists", async () => includes("src/accounting-routing.js", ["aggregateUsage", "byModel", "byRecipe", "overTime"]) && includes("server.js", ["/api/accounting/usage", "handleUsageDashboard"]) && includes("public/index.html", ["usageSummary", "Usage"])));
  checks.push(await check("Epic AM budget caps exist", async () => includes("src/accounting-routing.js", ["normalizeBudgetCaps", "evaluateBudgetCaps", "inputHardTokens", "outputHardTokens"]) && includes("server.js", ["budget-hard-limit", "budgetCaps"]) && includes("public/index.html", ["budgetProfileSelect"])));
  checks.push(await check("Epic AM automatic model routing exists", async () => includes("src/accounting-routing.js", ["chooseModelRoute", "classifyTaskType", "automatic model routing"]) && includes("server.js", ["/api/accounting/routing", "handleRoutingPreview"]) && includes("public/app.js", ["AUTO_MODEL_VALUE", "Auto route"])));
  checks.push(await check("Epic AM speculative draft verify exists", async () => includes("src/accounting-routing.js", ["draft-then-verify", "verifyModel", "speculative"]) && includes("server.js", ["maybeRunSpeculativeVerification", "speculative-verification"]) && includes("public/index.html", ["routingModeSelect", "Verify"])));
  checks.push(await check("Epic AM per-recipe default model exists", async () => includes("recipes/schema.json", ["defaultModel"]) && includes("server.js", ["recipe.defaultModel", "recipeForRun"]) && includes("recipes/diff-safe-coder.json", ["defaultModel"])));
  ...
  checks.push(await check("Epic AP public demo proof gate exists", async () => includes("package.json", ["demo:proof", "demo:health", "demo:workspace"]) && includes("docs/demo-proof.html", ["Trust Score 100 proof loop", "docs/demo-proof/search-results.json", "docs/demo-proof/reports/trust-loop-report.html"]) && includes("scripts/check-demo-proof.js", ["demo proof fingerprint is stale", "npm run demo:health"]) && includes("docs/RELEASE_PROCESS.md", ["npm run demo:health", "demo assets are stale"]) && hasFile("docs/demo-proof/manifest.json") && hasFile("docs/demo-proof/fingerprint.json")));
  ...
  checks.push(await check("Accounting and smart routing exist", async () => includes("src/accounting-routing.js", ["agenttrail.usage-record.v1", "chooseModelRoute", "timeToFirstTokenMs", "draft-then-verify"]) && includes("server.js", ["/api/accounting/usage", "maybeRunSpeculativeVerification", "timeToFirstTokenMs"]) && includes("public/index.html", ["usageSummary", "routingModeSelect", "budgetProfileSelect"]) && includes("tests/unit/accounting-routing.test.js", ["avgTimeToFirstTokenMs", "draft-then-verify"])));
  checks.push(await check("Automation webhook trigger exists", async () => includes("server.js", ["/api/webhooks/run", "handleWebhookRun", "AGENTTRAIL_WEBHOOK_TOKEN", "receipts/webhooks"]) && includes("src/route-catalog.js", ["/api/webhooks/run"]) && includes("docs/INTEGRATIONS.md", ["/api/webhooks/run", "AGENTTRAIL_WEBHOOK_TOKEN"]) && includes("tests/integration/api.test.js", ["/api/webhooks/run", "receiptPath"])));
  ...
  checks.push(await check("Bundled GGUF runtime adapter exists", async () => includes("src/model-adapters.js", ["bundled", "AGENTTRAIL_GGUF_MODEL"]) && includes("src/bundled-runtime.js", ["node-llama-cpp", "generateBundledText", "embedBundledText", "generateSpeculative", "preloadPrompt"]) && includes("scripts/validate-bundled-runtime.js", ["agenttrail.bundled-runtime-validation.v1", "generateBundledText", "embedBundledText"]) && includes("tests/integration/bundled-runtime.test.js", ["AGENTTRAIL_MODEL_ADAPTER", "bundled", "mock-gguf", "AGENTTRAIL_BUNDLED_SPECULATIVE"]) && includes("tests/integration/bundled-runtime-validation.test.js", ["validate-bundled-runtime", "speculative bundled runtime ok"]) && includes("docs/RUNTIME_PHASE6.md", ["AGENTTRAIL_MODEL_ADAPTER=bundled", "test:bundled", "validate:bundled-runtime"])));
  ...
  checks.push(await check("Runtime loading internals policy exists", async () => includes("src/runtime-loading.js", ["detectModelQuantization", "resolveKvCachePolicy", "resolvePrefillPolicy", "resolveSpeculativePolicy", "resolveBatchPolicy", "resolveMmapPolicy", "resolveShardingPolicy", "runtimeBenchmarkPlan"]) && includes("scripts/benchmark-runtime.js", ["tokensPerSecond", "generateBundledText", "/api/generate"]) && includes("tests/unit/runtime-loading.test.js", ["Q4_K_M", "AGENTTRAIL_TENSOR_SPLIT", "AGENTTRAIL_SPECULATIVE_DECODING"]) && includes("docs/RUNTIME_PHASE6.md", ["AGENTTRAIL_BUNDLED_MMAP", "bench:runtime", "AGENTTRAIL_SPECULATIVE_DECODING"])));
...
function categoryFor(name) {
  ...
  if (/epic am|accounting|usage dashboard|budget caps|automatic model routing|speculative|draft verify|per-recipe default model|token time/.test(text)) return "accounting-routing";
